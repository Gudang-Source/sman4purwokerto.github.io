...
$case=mysql_query("SELECT * FROM ".$DB_KODE."_case where nama_case='$formulasi' ");
$c=mysql_fetch_array($case);
//echo $c['nama_case'];
if ($formulasi==$c['nama_case']){

            if ($c['blok_case']==0){
            
        }elseif($c['blok_case']==1){
        $kiri=1;  
        }elseif($c['blok_case']==2){            
        $kanan=1;  
        }
    include "$c[file_case]";
}else{

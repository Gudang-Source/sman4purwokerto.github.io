...
if (isset($_SESSION['guru']) AND isset($_SESSION['nickname'])){
...
function Upload_B($fileName, $extensionList, $namaDir,$data){
	...
	$ekstensi = $pecah[1];
	$pathFile = $namaDir . $fileName;
	if (in_Brray($ekstensi, $extensionList)){		$tmpName  = $_FILES['fupload_B']['tmp_name'];
	global $data;
...
function Upload_C($fileName, $extensionList, $namaDir,$data){
	...
	$ekstensi = $pecah[1];
	$pathFile = $namaDir . $fileName;
	if (in_Crray($ekstensi, $extensionList)){		$tmpName  = $_FILES['fupload_C']['tmp_name'];
	global $data;
...
function Upload_D($fileName, $extensionList, $namaDir,$data){
	...
	$ekstensi = $pecah[1];
	$pathFile = $namaDir . $fileName;
	if (in_Drray($ekstensi, $extensionList)){		$tmpName  = $_FILES['fupload_D']['tmp_name'];
	global $data;
...
function Upload_E($fileName, $extensionList, $namaDir,$data){
	...
	$ekstensi = $pecah[1];
	$pathFile = $namaDir . $fileName;
	if (in_Erray($ekstensi, $extensionList)){		$tmpName  = $_FILES['fupload_E']['tmp_name'];
	global $data;

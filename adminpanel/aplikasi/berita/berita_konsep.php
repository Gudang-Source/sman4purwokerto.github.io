...
<?php 			include "aplikasi/berita/header.php"; ?>	
		...
		<div class="atastabel" style="margin-bottom: 10px">
			...
			<?php     echo "<form method='POST' action='$database?t=$token&pilih=konsep&untukdi=hapusbanyak'>"; ?>
			<div class="cari">
		...
		<table class="tabel" id="results">
			...
			$no=1;
			if($_SESSION['leveluser']=='0'){
			$databerita=mysql_query("SELECT * FROM ".$DB_KODE."_berita,".$DB_KODE."_users, ".$DB_KODE."_kategori WHERE ".$DB_KODE."_berita.s_username=".$DB_KODE."_users.s_username AND ".$DB_KODE."_berita.id_kategori=".$DB_KODE."_kategori.id_kategori AND status_terbit='0' ORDER BY id_berita DESC");
			}
			else {
			$databerita=mysql_query("SELECT * FROM ".$DB_KODE."_berita,".$DB_KODE."_users, ".$DB_KODE."_kategori WHERE ".$DB_KODE."_berita.s_username=".$DB_KODE."_users.s_username AND ".$DB_KODE."_berita.id_kategori=".$DB_KODE."_kategori.id_kategori AND status_terbit='0' AND ".$DB_KODE."_berita.s_username='$_SESSION[adminsh]' ORDER BY id_berita DESC");
			}
			...
			<tr class="tabel">
				...
				<td class="tabel" width="150px">
					<div class="hapusdata">
					<a href="<?php    echo "$database?t=$token&pilih=konsep&untukdi=hapus&id=$db[id_berita]";?>">hapus</a>
					</div>
					...
					<a href="<?php    echo "$database?t=$token&pilih=konsep&untukdi=terbitkan&id=$db[id_berita]";?>">terbitkan</a>
					</div>
					...
					<div class="bataldata">
					<a href="<?php    echo "$database?t=$token&pilih=konsep&untukdi=batalkan&id=$db[id_berita]";?>">batalkan</a>
					</div>
					<?php     }?>
				</td>
			</tr>

...
$database="aplikasi/berita/berita.php";
switch($_GET['pilih']){
default: ?>
<h3>Berita</h3>
...
<?php 			include "aplikasi/berita/header.php"; ?>	
		...
		<table class="tabel" id="results">
			...
			elseif($_GET['by']=="penulisasc") {$urutkan="".$DB_KODE."_berita.s_username ASC";}
			elseif($_GET['by']=="penulisdesc") {$urutkan="".$DB_KODE."_berita.s_username DESC";}
			elseif($_GET['by']=="tanggalasc") {$urutkan="tanggal_posting";}
			elseif($_GET['by']=="tanggaldesc") {$urutkan="tanggal_posting DESC";}
			elseif($_GET['by']=="kategoriasc") {$urutkan="".$DB_KODE."_kategori.nama_kategori ASC";}
			elseif($_GET['by']=="kategoridesc") {$urutkan="".$DB_KODE."_kategori.nama_kategori DESC";}
			else {$urutkan="id_berita DESC";}
			$no=1;
			if($_SESSION[leveluser]=='0'){
			$databerita=mysql_query("SELECT * FROM ".$DB_KODE."_berita,".$DB_KODE."_users, ".$DB_KODE."_kategori WHERE ".$DB_KODE."_berita.s_username=".$DB_KODE."_users.s_username AND ".$DB_KODE."_berita.id_kategori=".$DB_KODE."_kategori.id_kategori ORDER BY $urutkan");
			}
			else {
			$databerita=mysql_query("SELECT * FROM ".$DB_KODE."_berita,".$DB_KODE."_users, ".$DB_KODE."_kategori WHERE ".$DB_KODE."_berita.s_username=".$DB_KODE."_users.s_username AND ".$DB_KODE."_berita.id_kategori=".$DB_KODE."_kategori.id_kategori AND ".$DB_KODE."_berita.s_username='$_SESSION[adminsh]' ORDER BY id_berita DESC");
			}
			...
			<tr class="tabel">
				...
				<td class="tabel"><?php    echo "<a href='kategori.php?t=$token&pilih=edit&id=$db[id_kategori]'>$db[nama_kategori]</a>";?></td><!--Menampilkan kategori -->
    ...
    //--></script>
		...
		case "terbit":
			include "aplikasi/berita/berita_terbit.php";
		break;
		
		case "konsep":
			include "aplikasi/berita/berita_konsep.php";
		break;
		
		case "pencarian":
			include "aplikasi/berita/berita_pencarian.php";
		break;
		
		case "kategori":
			include "aplikasi/berita/berita_kategori.php";
		break;
		
		case "penulis":
			include "aplikasi/berita/berita_penulis.php";
		break;
		
		case "tambah":
			include "aplikasi/berita/berita_tambah.php";
		break;
		
		case "edit":
			include "aplikasi/berita/berita_edit.php";
		}
		?>

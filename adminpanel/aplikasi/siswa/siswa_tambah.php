...
<h3>Tambah<?php echo $_SESSION['Bsiswa'];?></h3>
...
<?php 			include "aplikasi/siswa/header.php"; ?>
		...
		<table class="isian">
		<form method='POST'<?php     echo "action='$database?t=$token&pilih=siswa&untukdi=tambah'";?> name='tambahsiswa' id='tambahsiswa'  enctype="multipart/form-data">
			
			<tr><td valign="top" class="isiankanan" width="175px"><?php echo $_SESSION['Bsiswa']; ?></td><td class="isian"><input type="text" name="nama_siswa" class="maksimal"></td></tr>
			<tr><td valign="top" class="isiankanan" width="175px">NIS</td><td class="isian"><input type="text" name="nis" class="pendek"></td></tr>
			<tr><td valign="top" class="isiankanan" width="175px">NISN</td><td class="isian"><input type="text" name="nisn" class="pendek"></td></tr>
...
<?php  if ($_SESSION['jenjang']>4){	?>				
			...
			<td class="isian">
					<select name="jurusan">
						...
						<?php    
							$jurusan=mysql_query("SELECT * FROM ".$DB_KODE."_jurusan");
							while($k=mysql_fetch_array($jurusan)){
							echo "<option value='$k[id_jurusan]'>$k[nama_jurusan]</option>";}
						?>
					</select>
			</td></tr>
<?php  } else {	?>	<input type="hidden" name="jurusan" value="1"><?php  } ?>			

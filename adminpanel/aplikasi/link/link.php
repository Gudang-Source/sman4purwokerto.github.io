...
if ($pilih=='link' AND $untukdi=='tambah'){
	...
	gogo('../../link.php?t='.$token.'');
}

elseif ($pilih=='link' AND $untukdi=='edit'){
	mysql_query("UPDATE ".$DB_KODE."_sidebar SET 	nama ='$_POST[nama_link]',
										isi1 ='$_POST[url_link]',
										isi2 ='$_POST[isi2]',										
...
elseif ($pilih=='link' AND $untukdi=='nonaktif'){
	mysql_query("UPDATE ".$DB_KODE."_sidebar SET 	status ='0' WHERE id_sidebar ='$id'");						
	gogo('../../link.php?t='.$token.'');
}
elseif ($pilih=='link' AND $untukdi=='aktif'){
	mysql_query("UPDATE ".$DB_KODE."_sidebar SET 	status ='1' WHERE id_sidebar ='$id'");						
	gogo('../../link.php?t='.$token.'');
}
elseif ($pilih=='link' AND $untukdi=='hapus'){
	mysql_query("DELETE FROM ".$DB_KODE."_sidebar WHERE id_sidebar ='$id'");					
	gogo('../../link.php?t='.$token.'');
}elseif ($pilih=='kategori' AND $untukdi=='tambah'){
	mysql_query("INSERT INTO ".$DB_KODE."_kategori
				(nama_kategori, deskripsi_kategori, fungsi, sub)
				VALUES
				('$_POST[nama_kat]','$_POST[deskripsi_kat]','link','$_POST[sub]')");
				
	gogo('../../link.php?t='.$token.'&pilih=kategori');
}

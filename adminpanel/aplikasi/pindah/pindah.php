...
if ($pilih=='pindah' AND $untukdi=='tambah'){
...
$namaDir = '../../../images/foto/siswa/';
	...
	gogo('../../pindah.php?t='.$token.'');
}

elseif ($pilih=='pindah' AND $untukdi=='edit'){
$lokasi_file = $_FILES['fupload']['tmp_name'];
$nama_file   = $_FILES['fupload']['name'];
	if (!empty($lokasi_file)){
$extensionList = array("bmp", "jpg", "gif", "png", "BMP", "JPG", "GIF", "PNG", "Bmp", "Jpg", "Gif", "Png");
$namaDir = '../../../images/foto/siswa/';
	...
	mysql_query("UPDATE ".$DB_KODE."_siswa SET 		nama_siswa ='$_POST[nama_pindah]',
											foto='$nama_file',
											jenkel='$_POST[jk]',
...
elseif ($pilih=='pindah' AND $untukdi=='hapus'){
$id=ceknomor($_GET['id']);
	mysql_query("DELETE FROM ".$DB_KODE."_siswa WHERE id_siswa ='$id'");					
	gogo('../../pindah.php?t='.$token.'');}
	

elseif ($pilih=='pindah' AND $untukdi=='hapusbanyak'){
	$cek=$_POST['cek'];
	$jumlah=count($cek);
	for($i=0;$i<$jumlah;$i++){

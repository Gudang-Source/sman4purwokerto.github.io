...
<?php 			include "aplikasi/komentar/header.php"; ?>		
		...
		<?php     echo "<form method='POST' action='$database?t=$token&pilih=tolak&untukdi=hapusbanyak'>"; ?>
		...
		<table class="tabel" id="results">
			...
			$no=1;
			if($_SESSION[leveluser]=='0'){
			$komentar=mysql_query("SELECT * FROM ".$DB_KODE."_komentar, ".$DB_KODE."_berita WHERE ".$DB_KODE."_komentar.id_berita=".$DB_KODE."_berita.id_berita AND status_terima='0' ORDER BY id_komentar DESC");
			}
			else {
			$komentar=mysql_query("SELECT * FROM ".$DB_KODE."_komentar, ".$DB_KODE."_berita WHERE ".$DB_KODE."_komentar.id_berita=".$DB_KODE."_berita.id_berita AND status_terima='0' AND ".$DB_KODE."_berita.s_username='$_SESSION[adminsh]' ORDER BY id_komentar DESC");
			}
			...
			<tr class="tabel">
				...
				<?php     
				echo "
					<div class='hapusdata'><a href='$database?t=$token&pilih=tolak&untukdi=hapus&id=$kom[id_komentar]'>hapus</a></div>
					<div class='editdata'><a href='?t=$token&pilih=edit&id=$kom[id_komentar]'>edit</a></div>
					<div class='terbitdata'><a href='$database?t=$token&pilih=tolak&untukdi=terima&id=$kom[id_komentar]'>terima</a></div>";
				?>
				</td>
			</tr>
			<?php    

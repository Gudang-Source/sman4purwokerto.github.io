...
<?php 			include "aplikasi/komentar/header.php"; ?>	
		...
		<table class="tabel" id="results">
			...
			$no=1;
			if($_SESSION[leveluser]=='0'){
			$komentar=mysql_query("SELECT * FROM ".$DB_KODE."_komentar, ".$DB_KODE."_berita WHERE ".$DB_KODE."_komentar.id_berita=".$DB_KODE."_berita.id_berita AND status_terima='1' ORDER BY id_komentar DESC");
			}
			else {
			$komentar=mysql_query("SELECT * FROM ".$DB_KODE."_komentar, ".$DB_KODE."_berita WHERE ".$DB_KODE."_komentar.id_berita=".$DB_KODE."_berita.id_berita AND status_terima='1' AND ".$DB_KODE."_berita.s_username='$_SESSION[adminsh]' ORDER BY id_komentar DESC");
			}
			...
			<tr class="tabel">
				...
				<?php     
				echo "
					<div class='hapusdata'><a href='$database?t=$token&pilih=terima&untukdi=hapus&id=$kom[id_komentar]'>hapus</a></div>
					<div class='editdata'><a href='?t=$token&pilih=edit&id=$kom[id_komentar]'>edit</a></div>
					<div class='bataldata'><a href='$database?t=$token&pilih=terima&untukdi=tolak&id=$kom[id_komentar]'>tolak</a></div>";
				?>
				</td>
			</tr>
			<?php    

...
$database="aplikasi/pengaturan/pengaturan.php";
 ?>
<h3>Pengaturan</h3>
<div class="isikanan"><!--Awal class isi kanan-->
								
		<div class="judulisikanan">
			<div class="menuhorisontalaktif-ujung"><a href="pengaturan.php?t=<?php echo $token; ?>">Website</a></div>
		...
		<table class="isian">
		<form method='POST' action="<?php    echo "$database?t=$token";?>" name='editpengaturan' id='editpengaturan' >
			<?php    
			$url=mysql_query("SELECT * FROM ".$DB_KODE."_pengaturan WHERE id_pengaturan=1");
			$u=mysql_fetch_array($url);
			?>
			<tr><td valign="top" class="isiankanan" width="350px">URL website</td><td class="isian"><input type="text" name="url" class="sedang" value ="<?php    echo "$u[isi_pengaturan]";?>"></td></tr>
			...
			<?php    
			$tambahadmin=mysql_query("SELECT * FROM ".$DB_KODE."_pengaturan WHERE id_pengaturan=2");
			$ta=mysql_fetch_array($tambahadmin);
			?>			
			<tr><td valign="top" class="isiankanan" width="100px">Perbolehkan Administrator menambah data admin lain</td><td class="isian">
			...
			?>
			<tr><td valign="top" class="isiankanan" width="100px">Jumlah data yang tampil pada setiap halaman admin</td><td class="isian"><input type="text" name="jml_data" class="pendek" value ="<?php    echo "$jt[isi_pengaturan]";?>"></td></tr>
			<tr><td class="isian" colspan="2" style="color:#7f7f7f;"><i>
			Masukkan jumlah data yang ditampilkan di setiap halaman atau modul yang ada di halaman admin
			</i><br><hr></td></tr>
			
			<?php    
			$siswa=mysql_query("SELECT * FROM ".$DB_KODE."_pengaturan WHERE id_pengaturan=4");
			$s=mysql_fetch_array($siswa);
			?>
			<tr><td valign="top" class="isiankanan" width="100px">Perbolehkan pengunjung melihat data<?php echo $_SESSION['Bsiswa']; ?></td><td class="isian">
			...
			<?php    
			$alumni=mysql_query("SELECT * FROM ".$DB_KODE."_pengaturan WHERE id_pengaturan=5");
			$a=mysql_fetch_array($alumni);
			?>
			<tr><td valign="top" class="isiankanan" width="100px">Perbolehkan pengunjung melihat data Alumni</td><td class="isian">
			...
			<?php    
			$guru=mysql_query("SELECT * FROM ".$DB_KODE."_pengaturan WHERE id_pengaturan=6");
			$g=mysql_fetch_array($guru);
			?>
			<tr><td valign="top" class="isiankanan" width="100px">Perbolehkan pengunjung melihat data<?php echo $_SESSION['Bguru']; ?></td><td class="isian">
			...
			?>
			<tr><td valign="top" class="isiankanan" width="100px">Tampilkan form input alumni pada halaman alumni</td><td class="isian">
			...
			<?php    
			$bukutamu=mysql_query("SELECT * FROM ".$DB_KODE."_pengaturan WHERE id_pengaturan=16");
			$bt=mysql_fetch_array($bukutamu);
			?>
			<tr><td valign="top" class="isiankanan" width="100px">Tampilkan data pesan di halaman buku tamu</td><td class="isian">

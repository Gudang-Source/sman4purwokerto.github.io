...
{
var answer = confirm("Apakah anda yakin ? karena semua isi database dari pendaftaran siswa akan terhapus semua!")
 if (answer)
 {
  return true;
 } else {
...
<div class="isikanan"><!--Awal class isi kanan-->
		...
		<?php     echo "<form method='POST' action='$database?t=$token&pilih=psb&untukdi=hapusbanyak'>";?>
		...
		<table class="tabel" id="results">
			...
			<?php    
			$no=1;
			$psb=mysql_query("SELECT * FROM ".$DB_KODE."_psb ORDER BY id_psb DESC");
			...
			<tr class="tabel">
				...
				<td class="tabel">
					<div class="hapusdata"><a href="<?php    echo "$database?t=$token&pilih=psb&untukdi=hapus&id=$p[id_psb]";?>">hapus</a></div>
					<div class="editdata"><a href="<?php    echo"?t=$token&pilih=edit&id=$p[id_psb]";?>">edit</a></div>
					<?php     if ($p[status_psb]==0){ ?>
					<div class="terbitdata"><a href="<?php    echo "$database?t=$token&pilih=psb&untukdi=terima&id=$p[id_psb]";?>">terima</a></div>
					<?php     }
					else { ?>
					<div class="bataldata"><a href="<?php    echo "$database?t=$token&pilih=psb&untukdi=tolak&id=$p[id_psb]";?>">tolak</a></div>
					<?php     } ?>
				</td>
			</tr>
			<?php    
    ...
    //--></script>
				...
				<div class="atastabel" style="margin-bottom: 10px">
							<form method='POST'<?php     echo"action='$database?t=$token&pilih=kosongkan&untukdi=hapus'";?> name='kosongkan' id='kosongkan' >
									<input type="submit" class="hapus"  value="KOSONGKAN DATA PSB" onclick="return confirmation();" >
					</form>
				</div>	
<!--Akhir class isi kanan-->
		...
		case "jenkel":
			include "aplikasi/psb/psb_jenkel.php";
		break;
		
		case "pencarian":
			include "aplikasi/psb/psb_pencarian.php";
		break;
		
		case "kosongkan":
			include "aplikasi/psb/psb_kosongkan.php";
		break;
		
		case "cetak":
			include "aplikasi/psb/psb_cetak.php";
		}
		?>

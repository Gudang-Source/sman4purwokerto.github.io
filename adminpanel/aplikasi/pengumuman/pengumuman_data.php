...
$database="aplikasi/pengumuman/pengumuman.php";
switch($_GET['pilih']){
default: ?>
<h3>Pengumuman</h3>
<div class="isikanan"><!--Awal class isi kanan-->
		...
		<?php     echo"<form method='POST' action='$database?t=$token&pilih=pengumuman&untukdi=hapusbanyak'>";?>
		...
		<table class="tabel" id="results">
			...
			$pengumuman=mysql_query("SELECT * FROM ".$DB_KODE."_pengumuman ORDER BY id_pengumuman DESC");}
			else {
			$pengumuman=mysql_query("SELECT * FROM ".$DB_KODE."_pengumuman WHERE s_username='$_SESSION[adminsh]' ORDER BY id_pengumuman DESC");
			}
			...
			<tr class="tabel">
				...
				<td class="tabel"><a href="<?php    echo "?t=$token&pilih=edit&id=$p[id_pengumuman]";?>"><b><?php    echo "$p[judul_pengumuman]";?></b></a></td>
				<td class="tabel"><?php    echo "$p[tanggal_pengumuman]";?></td>
				<td class="tabel"><?php   
				$penulis=mysql_query("SELECT * FROM ".$DB_KODE."_users WHERE s_username='$p[s_username]'");
				$pen=mysql_fetch_array($penulis);
				echo "$pen[nama_lengkap_users]";?></td>
				<td class="tabel">
				<?php     
				echo "
					<div class='hapusdata'><a href='$database?t=$token&pilih=pengumuman&untukdi=hapus&id=$p[id_pengumuman]'>hapus</a></div>
					<div class='editdata'><a href='?t=$token&pilih=edit&id=$p[id_pengumuman]'>edit</a></div>";
				?>
				</td>
			</tr>
    ...
    //--></script>
		...
		case "tambah":
			include "aplikasi/pengumuman/pengumuman_tambah.php";
		break;
		
		case "edit":
			include "aplikasi/pengumuman/pengumuman_edit.php";
		}
		?>

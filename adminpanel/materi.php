...
<?php 		$dir='elearning';
		...
		if($r2s['status']==1){	
		unset($_SESSION['dir']);
		unset($_SESSION['webbx']);
		if (!isset($_SESSION['webbx']))	{
					$_SESSION['webbx']=$r2s['session'];
		}		
?>	
<?php global $la;  if($la['pbm']==1){ ?>					
					<?php     include "plugin/".$dir."/materi/materi_data.php"; ?>
...
<?php global $la;  if($la['pbm']==1){ ?>					
					<?php     include "aplikasi/materi/materi_data.php"; ?>
<?php  }else{ echo"<center><b>Ma'af Anda tidak diijinkan mengkases halaman ini<b><br>silahkan hubungi administrator bila ingin mengakses halaman ini.</center>";} ?>	
